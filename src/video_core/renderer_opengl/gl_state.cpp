...
template <typename T>
inline bool UpdateValue(T& current_value, const T new_value) {
...
inline bool UpdateTie(T1 current_value, const T2 new_value) {
    const bool changed = current_value != new_value;
    current_value = new_value;
    return changed;
}

inline void Enable(GLenum cap, bool enable) {
    if (enable) {
        glEnable(cap);
    } else {
        glDisable(cap);
...
inline void Enable(GLenum cap, GLuint index, bool enable) {
    if (enable) {
        glEnablei(cap, index);
    } else {
...
inline void Enable(GLenum cap, bool& current_value, bool new_value) {
    if (UpdateValue(current_value, new_value))
        Enable(cap, new_value);
}

inline void Enable(GLenum cap, GLuint index, bool& current_value, bool new_value) {
    if (UpdateValue(current_value, new_value))
