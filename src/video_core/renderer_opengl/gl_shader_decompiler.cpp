...
private:
    ...
    size_t ValidateAndGetCoordinateElement(const Tegra::Shader::TextureType& texture_type,
                                           const bool depth_compare, const bool is_array,
                                           const bool lod_bias_enabled, size_t max_coords,
                                           size_t max_inputs, std::string& coord) {
        ...
        static const char* coord_container[] = {
            "", "float coord = (", "vec2 coord = vec2(", "vec3 coord = vec3(", "vec4 coord = vec4(",
        };
        coord = coord_container[total_coord_count];
        return coord_count;
    }
    ...
    std::string GetTextureCode(const Tegra::Shader::Instruction& instr,
        ...
        const std::string read_method = tex_method == Tegra::Shader::TextureProcessMode::LZ ||
    ...
    void SetTEXCode(const Instruction& instr, const Tegra::Shader::TextureType& texture_type,
                    const bool depth_compare, const bool is_array,
                    const Tegra::Shader::TextureProcessMode& tex_method, std::string& texture,
                    std::string& coord) {
        const bool lod_bias_enabled = !(tex_method == Tegra::Shader::TextureProcessMode::None ||
                                        tex_method == Tegra::Shader::TextureProcessMode::LZ);
        const size_t coord_count = ValidateAndGetCoordinateElement(
            texture_type, depth_compare, is_array, lod_bias_enabled, 4, 5, coord);
        ...
        coord += ");";
        texture = GetTextureCode(instr, texture_type, depth_compare, is_array, tex_method, 0);
    }

    void SetTEXSCode(const Instruction& instr, const Tegra::Shader::TextureType& texture_type,
                     const bool depth_compare, const bool is_array,
                     const Tegra::Shader::TextureProcessMode& tex_method, std::string& texture,
                     std::string& coord) {
        const bool lod_bias_enabled = !(tex_method == Tegra::Shader::TextureProcessMode::None ||
                                        tex_method == Tegra::Shader::TextureProcessMode::LZ);
        const size_t coord_count = ValidateAndGetCoordinateElement(
            texture_type, depth_compare, is_array, lod_bias_enabled, 4, 4, coord);
        // If enabled arrays index is always stored in the gpr8 field
        const u64 array_register = instr.gpr8.Value();
        ...
        for (size_t i = 0; i < coord_count; ++i) {
            const bool last = i == (coord_count - 1);
        ...
        coord += ");";
        texture = GetTextureCode(instr, texture_type, depth_compare, is_array, tex_method,
                                 (coord_count > 2 ? 1 : 0));
    }
    ...
    u32 CompileInstr(u32 offset) {
        ...
        case OpCode::Type::Memory: {
            ...
            case OpCode::Id::TEX: {
                ...
                SetTEXCode(instr, texture_type, depth_compare, is_array, process_mode, texture,
                           coord);
            ...
            case OpCode::Id::TEXS: {
                ...
                SetTEXSCode(instr, texture_type, depth_compare, is_array, process_mode, texture,
                            coord);
