...
void CachedSurface::LoadGLBuffer() {
    ...
    if (params.is_tiled) {
        ...
        for (size_t depth = 0; depth < params.depth; ++depth) {
            const size_t offset{depth * copy_size};
...
Surface RasterizerCacheOpenGL::RecreateSurface(const Surface& surface,
    ...
    if (params.pixel_format == new_params.pixel_format && params.target == new_params.target &&
        params.depth == new_params.depth && params.block_height == new_params.block_height) {
        ...
        // Otherwise, always copy old data to new surface, regardless of format
