...
constexpr double MAX_DELAY_TIME = 0.7;             // Units: seconds
constexpr size_t DROP_FRAMES_SAMPLE_DELAY = 24000; // Units: samples
...
void TimeStretcher::SetNativeSampleRate(unsigned int sample_rate) {
    impl->sample_rate = static_cast<double>(sample_rate);
    impl->soundtouch.setSampleRate(impl->sample_rate);
    SetOutputSampleRate(impl->sample_rate);
}

void TimeStretcher::SetOutputSampleRate(unsigned int sample_rate) {
    impl->sample_rate = static_cast<double>(sample_rate);
    impl->soundtouch.setRate(1);
}

void TimeStretcher::SetNumChannels(u32 num_channels) {
    impl->num_channels = num_channels;
    impl->soundtouch.setChannels(impl->num_channels);
}
...
double TimeStretcher::CalculateCurrentRatio() {
    ...
    const double expected_time =
        static_cast<double>(impl->samples_queued) / static_cast<double>(impl->sample_rate);
...
std::vector<s16> TimeStretcher::GetSamples() {
    ...
    std::vector<s16> output(static_cast<size_t>(available) * impl->num_channels);
